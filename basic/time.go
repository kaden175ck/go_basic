...
func basicTime() {
	...
	t1 := time.Now()
	diff1 := t1.Sub(t0)
	fmt.Printf("时间差%f", diff1.Seconds()) //计算时间差，返回类型是time.Duration
	fmt.Println()
	diff2 := time.Since(t0)
	fmt.Printf("时间差%f", diff2.Seconds()) //等价于t1 := time.Now();t1.Sub(t0)
	fmt.Println()
	...
	fmt.Printf("%d-%d-%d %d:%d:%d.%d\n", t2.Year(), t2.Month(), t2.Day(), t2.Hour(), t2.Minute(), t2.Second(), t2.UnixMilli()%t2.Unix())
}
...
func parse_format() {
	...
	t, _ = time.ParseInLocation(TIME_FMT, ts, loc) //生产环境一律使用ParseInLocation
	if t.Unix() != now.Unix() {
